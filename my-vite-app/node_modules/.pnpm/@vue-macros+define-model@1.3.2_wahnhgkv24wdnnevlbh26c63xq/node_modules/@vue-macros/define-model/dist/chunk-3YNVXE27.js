...
import _DM_useVModel from '${useVmodelHelperId}';`
      ...
      const text = `_DM_useVModel(${Object.entries(map).map(([name2, { options }]) => {
        const prop = getPropKey(name2, true);
        const evt = getEventKey(name2, true);
        if (!prop && !evt && !options)
          return stringifyValue(name2);
        const args = [name2, prop, evt].map((arg) => stringifyValue(arg));
        if (options) {
